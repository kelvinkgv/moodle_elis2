...
function draw_set($order, $set, $offset) {
  ...
  @$this->init_variable($stacker,    $this->y_format[$set]['stacker'],    '0');
  ...
  foreach ($this->x_data as $index => $x) {
    ...
    if (($bar!='none') && (string)$thisY != 'none') {
        ...
        if($colour=='multicolour'){
            $tcolour = 'col'.str_replace(' ','_',$x); 
        } else {
            $tcolour = $colour;
        }                  // IGC
        $this->bar($thisX, $thisY, $bar, $barSize, $tcolour, $offset, $set, $yoffset, $stacker);   // Moodle
    }
...
function init_y_axis() {
  ...
  $grain            = $this->parameter['y_grain_left'];
  ...
  $minLeft = (round($dataLeftRange['min']/$grain,0))*$grain;
  $maxLeft = (round($dataLeftRange['max']/$grain,0)+1)*$grain;
...
function init_colours() {
  ...
  $this->colour['amber']   = ImageColorAllocate ($this->image, 0xFF, 0xBF, 0x00);
