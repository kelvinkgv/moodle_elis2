...
class elis_plugin_rewrite{ //extends block_base {

	function __construct() {
        global $PAGE;
        $this->title		= get_string('rewrite_activity', 'block_elis2');
        $this->act_prefix	= 'rewrite';
        $this->isbn			= '';
        $this->explain_text 		= 'Rewrite the final 80-100 words of the story to provide a slightly different conclusion.';
	...
	function render_form($show_result=false,$uid=''){
		...
		list($r_js,$r_html) = elis2_lib::activityTextboxRow('Rewrite the final 80-100 words of the story to provide a slightly different conclusion.:',$this->act_prefix.'_q1',$this->isbn,true,80,100,$uid,$show_result);
		$html.=$r_html;
