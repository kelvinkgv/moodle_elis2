...
class elis_plugin_song{ //extends block_base {

	function __construct() {
        global $PAGE;
        $this->title		= get_string('song_activity', 'block_elis2');
        $this->act_prefix	= 'song';
        $this->isbn			= '';
        $this->explain_text 		= 'What music links best with your book?';
	...
	function render_form($show_result=false,$uid=''){
		...
		list($r_js,$r_html) = elis2_lib::activityTextboxRow('Provide a link to a song that you feel fits in with the mood and themes of your book:',$this->act_prefix.'_q1',$this->isbn,'','','',$uid,$show_result);
		$html.=$r_html;
		
		list($r_js,$r_html) = elis2_lib::activityTextboxRow('Explain why you have chosen it in 50-70 words.:',$this->act_prefix.'_q2',$this->isbn,true,50,70,$uid,$show_result);
		$html.=$r_html;
